...
var builtinPredicates = []RelationInfo{
	...
	{Name: "route", Args: []string{"from", "net", "path"}, ArgKinds: map[string]facts.ArgKind{"from": {Entity: check.KindNet}, "net": {Entity: check.KindNet}}, Summary: "the same walk as `reaches`, with the route it found bound as a readable value (`VBUS -[R5]- VBUS_F -[L1]- VDD_3V3`), so a connectivity answer carries the evidence for itself; one route per pair, and a route never ends on a rail because the walk refuses one", Kind: KindPredicate},
