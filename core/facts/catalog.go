...
type ArgKind struct {
	...
	//
	// Named for what it holds rather than "domain", which this repo already uses for a rule's
	// CONSIDERED SET (query.Domain). Two meanings of domain in one query engine is a collision, and
	// the other one is the load-bearing concept.
	//
