...
// Relations returns the discoverable RELATION set: the built-in relations plus any
// overlay-registered ones, each overlay entry with synthesized arg labels from its field layout.
// Order is registration order and is not sorted here — a caller that also has predicates to show
// merges both lists and sorts once (see query.Catalog).
//
// Each entry's Detail is resolved through the registered doc resolver, so an undocumented relation
// still lists with its Summary.
func Relations() []RelationInfo {
	mu.RLock()
	defer mu.RUnlock()
	out := make([]RelationInfo, 0, len(builtinCatalog)+len(order))
	...
	for i := range out {
		out[i].Detail = doc(out[i].Name)
	}
	return out
}

// Doc resolves a relation's reference markdown, or "" when no catalog is installed or the relation
// has no doc.
...
// doc is the lock-free half of Doc, for callers already holding the lock.
func doc(name string) string {
	if builtinDoc == nil {
		return ""
	}
	return builtinDoc(name)
}
