...
func indexFindings(d *ir.Design) map[string]Finding {
...
func TestFindingKind(t *testing.T) {
	f := indexFindings(ruleFixture())
	if got := f["single-pin-net|STUB"].Kind; got != KindNet {
		t.Errorf("single-pin-net STUB kind = %q, want %q", got, KindNet)
	}
	if got := f["i2c-pull-up|SDA"].Kind; got != KindNet {
		t.Errorf("i2c-pull-up SDA kind = %q, want %q", got, KindNet)
	}
	if got := f["unconnected-component|R9"].Kind; got != KindComponent {
		t.Errorf("unconnected-component R9 kind = %q, want %q", got, KindComponent)
	}
}
...
func TestDuplicateRefDes(t *testing.T) {
	...
	if got.Kind != KindComponent {
		t.Errorf("kind = %q, want %q", got.Kind, KindComponent)
	}
	...
	if fs := RunDesign(&ir.Design{}); len(fs) != 0 {
...
func TestDanglingEndpoint(t *testing.T) {
	...
	if got.Kind != KindEndpoint {
		t.Errorf("kind = %q, want %q", got.Kind, KindEndpoint)
	}
