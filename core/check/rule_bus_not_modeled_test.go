...
func TestBusNotModeledResolution(t *testing.T) {
	...
	if fs := busNotModeled.Eval(NewModel(design([]*ir.BusNotModeled{bus("DATA[1:0]", "DATA0", "DATA1")}, "DATA0", "DATA1"))); len(fs) != 0 {
		t.Errorf("resolved bus should be silent, got %d findings", len(fs))
	}
	// One member missing -> fires, named after the bus.
	fs := busNotModeled.Eval(NewModel(design([]*ir.BusNotModeled{bus("DATA[1:0]", "DATA0", "DATA1")}, "DATA0")))
	...
	if fs := busNotModeled.Eval(NewModel(design([]*ir.BusNotModeled{{Kind: "geda_bus", Label: "B"}}))); len(fs) != 1 {
