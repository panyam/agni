...
// VDD pin on the given net, joined to a spec via the given identity channel.
...
func runSupplyRule(t *testing.T, d *ir.Design, set param.ParamSet) []Finding {
	t.Helper()
	m := NewModelWithParams(d, nil, set)
	return Run(m, []*Rule{supplyExceedsAbsMax})
}
...
func TestSupplyExceedsAbsMax(t *testing.T) {
	...
	f := fires[0]
	if f.Kind != KindComponent || f.Subject != "U1" {
...
func TestParamProviderMockBackend(t *testing.T) {
	...
	m := NewModelWithParams(supplyDesign("+5V", false, "ACME-33"), nil, mock)
	if fs := Run(m, []*Rule{supplyExceedsAbsMax}); len(fs) != 1 {
	...
	if fs := Run(NewModel(supplyDesign("+5V", false, "ACME-33")), []*Rule{supplyExceedsAbsMax}); len(fs) != 0 {
