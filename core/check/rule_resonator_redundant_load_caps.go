...
var resonatorRedundantLoadCaps = &Rule{
	...
	Eval: func(m Model) []Finding {
		...
		for _, c := range m.Components() {
			if m.HasClass(c.RefDes, ClassCeramicResonator) {
		...
		for _, ref := range order {
			...
			for _, name := range names {
				...
				var capRef string
				for _, conn := range n.Connections {
					if conn.ComponentRef != ref && m.HasClass(conn.ComponentRef, ClassCapacitor) && groundRef[conn.ComponentRef] {
