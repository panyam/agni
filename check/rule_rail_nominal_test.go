...
func runRailNominal(t *testing.T, d *ir.Design, set param.ParamSet) []Finding {
	t.Helper()
	return Run(NewModelWithParams(d, nil, set), []*Rule{railNominalOutOfRecommended})
}
...
func TestRailNominalGating(t *testing.T) {
	if ok, reason := Available(railNominalOutOfRecommended, nil); ok || !strings.Contains(reason, "--params") {
