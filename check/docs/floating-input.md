...
![Input-only net is flagged as floating; the same input pulled to a rail is fine](images/floating-input.svg)
...
one real automotive netlist that was 36 findings, every one a diode network.)
