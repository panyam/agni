...
func drcFindings(t *testing.T) map[string][]string {
	t.Helper()
	m := NewModelWithBoard(&ir.Design{}, drcBoard())
	got := map[string][]string{}
	for _, r := range []*Rule{trackWidth, holeSize, annularWidth, copperClearance} {
...
func TestBoardRuleAvailability(t *testing.T) {
	if ok, _ := Available(trackWidth, NewModel(&ir.Design{SourceFormat: "edif"})); ok {
		t.Error("board rule available on a netlist-only design")
	}
	if ok, reason := Available(trackWidth, NewModel(&ir.Design{SourceFormat: "kicad-pcb"})); !ok {
		t.Errorf("board rule unavailable on a board design: %s", reason)
	}
	if ok, reason := Available(trackWidth, NewModel(&ir.Design{SourceFormat: "ipc-2581"})); !ok {
		t.Errorf("board rule unavailable on an IPC-2581 board design: %s", reason)
	}
	if ok, _ := Available(trackWidth, nil); !ok {
	...
	if ok, reason := Available(trackWidth, NewModelWithBoard(&ir.Design{SourceFormat: "edif"}, drcBoard())); !ok {
