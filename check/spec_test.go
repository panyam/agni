...
func rulesByName() map[string]*Rule {
...
func TestSpecRule(t *testing.T) {
	s := &Spec{
		Over:    "nets",
		Where:   Cmp{L: Fact{"net.pin_count"}, Op: "==", R: Lit{0}},
		Message: "net {net.names:q} has no connections",
	}
	r := s.Rule(Rule{Name: "empty-net", Severity: "info"})
	...
	fs := r.Eval(NewModel(&ir.Design{Nets: []*ir.Net{tnet("EMPTY"), tnet("OK", "R1.1")}}))
	...
	(&Spec{Over: "nets", Where: IsTrue{T: Fact{"no.such.fact"}}}).Rule(Rule{Name: "bad"})
}
...
func TestSpecValidate(t *testing.T) {
	cases := []struct {
		...
		{"unknown over", &Spec{Over: "sheets"}},
		{"unknown fact", &Spec{Over: "nets", Where: IsTrue{T: Fact{"net.bogus"}}}},
		{"unbound var", &Spec{Over: "nets", Where: IsTrue{T: Var{"x"}}}},
		{"unregistered func", &Spec{Over: "nets", Where: IsTrue{T: Call{Fn: "nope"}}}},
		{"unknown collection", &Spec{Over: "nets", Where: ExistsIn{Over: "net.pins"}}},
		{"unknown collection in count", &Spec{Over: "nets", Where: Cmp{L: CountOf{Over: "net.pins"}, Op: ">", R: Lit{0}}}},
		{"bad operator", &Spec{Over: "nets", Where: Cmp{L: Lit{1}, Op: "~=", R: Lit{1}}}},
		{"bad pattern", &Spec{Over: "nets", Where: Match{T: Fact{"net.names"}, Pattern: "("}}},
		{"bad placeholder", &Spec{Over: "nets", Message: "{nope}"}},
		{"bad node in let", &Spec{Over: "nets", Let: map[string]Term{"x": Fact{"net.bogus"}}}},
	}
...
func TestSpecScopeRestore(t *testing.T) {
	s := &Spec{
		Over: "nets",
		Where: Cmp{
			L: CountOf{Over: "net.connections", Where: And{Xs: []Expr{
				Cmp{L: Fact{"pin.electrical_type"}, Op: "==", R: Lit{"output"}},
				ExistsIn{Over: "net.connections", Where: Cmp{L: Fact{"pin.electrical_type"}, Op: "==", R: Lit{"output"}}},
			}}},
			Op: "==", R: Lit{2},
		},
		Message: "x",
