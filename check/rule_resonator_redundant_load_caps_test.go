...
func TestResonatorRedundantLoadCaps(t *testing.T) {
	fs := resonatorRedundantLoadCaps.Eval(NewModel(resonatorFixture()))
	if len(fs) != 1 || fs[0].Subject != "Y1" || fs[0].Kind != KindComponent {
...
func TestResonatorRedundantSilentWithoutSeededClass(t *testing.T) {
	...
	if fs := resonatorRedundantLoadCaps.Eval(NewModel(d)); len(fs) != 0 {
