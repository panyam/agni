...
func fireSubjects(t *testing.T, r *Rule, nets ...*ir.Net) map[string]string {
	t.Helper()
	got := map[string]string{}
	for _, f := range r.Eval(NewModel(&ir.Design{Nets: nets})) {
...
func TestDuplicateNetName(t *testing.T) {
	fs := duplicateNetName.Eval(NewModel(&ir.Design{Nets: []*ir.Net{
