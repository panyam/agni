...
var builtinCatalog = []facts.RelationInfo{
	...
	{Name: "net.nominal_voltage", Args: []string{"net", "volts"}, ArgKinds: map[string]facts.ArgKind{"net": {Entity: check.KindNet}}, Summary: "a RAIL's nominal voltage derived from its net name (3V3 -> 3.3). Rails only; a non-rail net's name-derived level is net.signal_level", Kind: facts.KindNetlist},
	{Name: "net.signal_level", Args: []string{"net", "volts"}, ArgKinds: map[string]facts.ArgKind{"net": {Entity: check.KindNet}}, Summary: "the signalling level a NON-RAIL net's name declares, the other half of net.nominal_voltage. A house convention that encodes a level into a signal net's name lands here rather than being read as a rail nominal", Kind: facts.KindNetlist},
