...
![Two component outputs on one net are flagged; one output driving one input is fine](images/output-output-conflict.svg)

### Scope note

Counts hard drivers only, meaning outputs and power-outputs. Bidirectional (INOUT) pins are
...
one net), which is one driver electrically. Two-plus driving pins of the SAME component never fire;
