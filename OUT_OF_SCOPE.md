...
| `agni check` still exits 1 for both a tripped `--fail-on` gate and a failed run, where `review` now distinguishes a gate trip as 2. The two commands disagree until it is aligned | issue 199 PR | filed as its own issue; a behavioural change to a documented CI surface taught by two tutorial rungs did not belong inside a review PR |
