...
| `serve`'s design LOADER still reads exactly the ref it is given, so the descriptor-driven entry resolution the CLI now does is not applied server-side. Deliberate: the served client holds a resource model (`ProjectService.ResolveDesign`) and can act on it explicitly, where silently swapping the file behind `GetDesign` would change what the viewer shows without the viewer knowing | agni issue 170 PR1 | the web surfaces are wired (the same issue's client half); decide there whether the viewer resolves and shows it, rather than making the loader do it invisibly |
| `project.Tree` walks the mounts on every listing with no cache. Correct by construction (an operator edits a descriptor while the server runs, and a stale index would answer with a design's old entry) but it is a walk per `ListProjects` | agni issue 170 PR1 | a deployment turns up whose mounts are large enough for the walk to show; the port already hides it, so a caching or index-backed `service.ProjectStore` is a drop-in with no service change |
