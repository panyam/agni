...
func TestOsPartSpecStoreCAS(t *testing.T) {
	...
	spec, ver, found, err := st.Get(ctx, "m", "d.pdf")
	...
	if _, err := st.Save(ctx, "m", "d.pdf", &parampb.PartSpec{Mpn: "LM1117"}, "stale"); !errors.Is(err, service.ErrConflict) {
		t.Fatalf("first write with non-empty base should conflict, got %v", err)
	}
	v1, err := st.Save(ctx, "m", "d.pdf", &parampb.PartSpec{Mpn: "LM1117"}, "")
	if err != nil || v1 == "" {
		t.Fatalf("first write: v=%q err=%v", v1, err)
	}

	// Read back matches and returns the same version token.
	got, ver2, found, err := st.Get(ctx, "m", "d.pdf")
	...
	if _, err := st.Save(ctx, "m", "d.pdf", &parampb.PartSpec{Mpn: "X"}, ""); !errors.Is(err, service.ErrConflict) {
		t.Fatalf("empty base against an existing file should conflict, got %v", err)
	}
	v2, err := st.Save(ctx, "m", "d.pdf", &parampb.PartSpec{Mpn: "LM1117I"}, v1)
	if err != nil || v2 == v1 {
