...
[Getting started](getting-started.md) ran the whole catalog and read the first few
...
check. See "silence is not a pass" in [Concepts](concepts.md).
...
- `provenance.sourceFile` is the file the fact came from. For datasheet-backed rules the
  message also names the datasheet page and table (see [Datasheets](datasheets.md)).
...
rules on top of the built-ins; [Naming conventions](naming-conventions.md) is the simplest
...
- [Datasheets](datasheets.md): turn on the rules that compare your design against a part's
  real limits.
- [Comparing revisions](comparing-revisions.md): diff two versions of a design.
