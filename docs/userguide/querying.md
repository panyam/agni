...
The idea in one line: the tool turns your design into a set of simple facts — a net has a voltage,
a part sits on a net, a datasheet lists a limit, a copper track has a width — and a query is a
question over those facts.
...
| `param(mpn, symbol, value)` | a datasheet limit | `--params` (see [Datasheets](datasheets.md)) |
...
edit it. Every rung names its concept, with a one-line analogy for the SQL-literate; the repo-wide
software↔hardware map is [ANALOGY.md](../ANALOGY.md).
...
Reusing `?net` in two facts means "the same net in both" — that is a `JOIN ... ON`. This is the load-
bearing idea: joins are how you connect what a part is, where it sits, and what its rail carries.
...
after the filter", which no per-net question can see across a series element ([reach-walk
diagram](../analogy/reach-walk.svg)).

## Going further

### Find parts stressed above their datasheet rating

This is the flagship: join what the part is (`component.mpn`), what its datasheet says
(`param`), where it sits (`component-on-net`), and the rail's voltage (`net.max_voltage`), then keep
only the ones where the rated maximum is below the rail.
...
No other tool lets you ask a question that spans copper, connectivity, and datasheets at once, with
each answer traceable back to the layer and the datasheet page.
...
leaving the design. Datasheet (`param`) facts are not yet wired into the viewer — a query over
`param` returns nothing there — so datasheet joins stay on the CLI for now.
...
cannot trace. That is the whole point: a search you can trust because you can verify it. In the
viewer the provenance rides behind each row's expand toggle instead of a trailing column.

## What a query is not

A query **reports**, it does not judge. It has no notion of pass/fail — that is what
[checks](checks-and-reports.md) are for. If you find yourself running the same query to catch a
