...
The idea, in one line: transcribe a part's limits once (the Absolute Maximum Ratings, the
operating range) into a small file, and the tool compares every design that uses the part
against them. See "datasheets as checkable data" in [Concepts](concepts.md).
...
are not copyrightable; cite the document revision and page). There is also a pipeline that
...
- [Checks and reports](checks-and-reports.md): the general report-reading flow these
  findings appear in.
- [CLI reference](cli-reference.md): `--params` and the other flags.
