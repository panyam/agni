...
vocabulary from [Concepts](concepts.md) (what a *finding*, a *tier*, and *provenance* mean);
...
the "silence is not a pass" idea from [Concepts](concepts.md): a real all-clear names how
...
- [Checks and reports](checks-and-reports.md): narrow to one rule or category, read the
  full report, and follow a finding's provenance.
- [Datasheets](datasheets.md): add a parameter set so checks can compare your design
  against a part's real limits.
- [CLI reference](cli-reference.md): the full command and flag surface.
