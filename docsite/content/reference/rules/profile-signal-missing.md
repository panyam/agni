...
— its anchor signal (e.g. SPI-NOR's chip-select) is present — but a required signal net is absent.
...
limit: if the anchor itself is absent — a wholly-missing interface — there is nothing to anchor on,
