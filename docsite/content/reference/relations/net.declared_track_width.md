---
title: "net.declared_track_width"
description: "the track width a net SHOULD route at, cascaded across its classes by priority (join this, not the per-class rows)"
---
...
clearance from one class and its track width from another, which is exactly what the tool does when it routes.
...
net.declared_track_width(?n, ?_)` reads as unconstrained.
...
```
net.declared_track_width(?net, ?mm) => ?net, ?mm
```

Nets routed narrower than their own project demands:

```
net.declared_track_width(?net, ?declared), board.track_width(?net, ?actual), ?actual < ?declared => ?net, ?actual, ?declared
```
